...
class AnnotatedLayer {
    constructor() {
        ...
        this.annotations = /** @type {!Array<!Annotation>} */ [];
    }
    ...
    id_filtered(predicate) {
        let newAnnotatedLayer = new AnnotatedLayer();
        for (let op of this.id_ops.values()) {
            if (predicate(op)) {
                newAnnotatedLayer.put(op);
            }
        }
        for (let op of this.markers) {
            if (predicate(op)) {
                newAnnotatedLayer.markers.push(op);
            }
        }
        return newAnnotatedLayer;
    }
