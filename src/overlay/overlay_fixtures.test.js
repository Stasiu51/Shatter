...
test("overlay.fixtures_valid_contains_all_kinds", () => {
  ...
  // Sheets
  assertThat(/##!\s+SHEET\s+NAME=UPPER/.test(txt)).isEqualTo(true);
  assertThat(/##!\s+SHEET\s+NAME=MIDDLE/.test(txt)).isEqualTo(true);
  assertThat(/##!\s+SHEET\s+NAME=LOWER/.test(txt)).isEqualTo(true);
  // Embedding
  assertThat(/##!\s+EMBEDDING\s+TYPE=TORUS\s+LX=12\s+LY=12/.test(txt)).isEqualTo(true);
  // Layouts
  assertThat(/##!\s+LAYOUT\s+Q=@hex\s+SHEET=UPPER\s+MAP=HEX/.test(txt)).isEqualTo(true);
  assertThat(/##!\s+LAYOUT\s+Q=0..8\s+SHEET=LOWER\s+MAP=GRID/.test(txt)).isEqualTo(true);
  assertThat(/##!\s+LAYOUT\s+Q=0..2\s+SHEET=MIDDLE\s+MAP=LINE/.test(txt)).isEqualTo(true);
  // QUBIT overrides
  ...
  assertThat(/##!\s+POLY\s+SHEET=UPPER/.test(txt)).isEqualTo(true);
